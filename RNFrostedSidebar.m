...
- (void)didTapItemAtIndex:(NSUInteger)index {
    ...
    if (self.borderColors) {
        ...
        //NSLog(@"index %d",[self.selectedIndices firstIndex]);
        
        //gisxy modify
        if ([self.selectedIndices firstIndex]==index) {
            UIView *tempView=self.itemViews[index];
        ...
        else
        {
            [self.selectedIndices enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop)
             {
                 UIView *tempView=self.itemViews[idx];
                 tempView.layer.borderColor=[UIColor clearColor].CGColor;
                 [self.selectedIndices removeIndex:idx];
